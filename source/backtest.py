...
def run_backtest(portfolio, stock_prices, start, end, test_length = 1):
    stocks = portfolio[portfolio.stock>0].index
    stock_data = stock_prices[stocks]
   
    date_diff = end - start
    n_test = int(date_diff.days/365)
    start_y = start
    cash_paths = []
    for y in range(n_test):
        start_y = start + dt.timedelta(365*y)
        end_y = start_y + dt.timedelta(365)
